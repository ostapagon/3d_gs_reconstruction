...
def visualize_cameras(R_c2w, T_c2w, fig=None, show=True):
    if fig is None:
        fig = go.Figure()
    ...
    for r_c2w, t_c2w in zip(R_c2w, T_c2w):
        ...
        for v, c, n in zip([x, y, z], ['red', 'green', 'blue'], ['x', 'y', 'z']):
            fig.add_trace(go.Scatter3d(
            x=[t_c2w[0], t_c2w[0] + v[0]], 
            y=[t_c2w[1], t_c2w[1] + v[1]], 
            z=[t_c2w[2], t_c2w[2] + v[2]],
            mode='lines',
...
def visualize_cameras_trajectorie(T_c2w, fig=None, show=True):
    if fig is None:
        fig = go.Figure()
